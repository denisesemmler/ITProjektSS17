...
public class ReportServiceEntry implements EntryPoint {
	...
	private Label loginLabel = new Label("Log in with your Google-Account to access our Report Side");
	...
	// private final ProjektAdministrationAsync pr0jectAdmin =
	...
	public void onModuleLoad() {
		...
		loginService.login(GWT.getHostPageBaseURL() + "Report.html", new AsyncCallback<Teilnehmer>() {
				...
				if (result.isLoggedIn()) {
					...
					zurueck.addClickHandler(new ClickHandler() {
						public void onClick(ClickEvent e) {
							...
							//Routing zum Report Generator--> protocol (http:) + "//" + host (localhost:8888) + /ITProjektSS17local.html
							Window.open(Window.Location.getProtocol()+"//"+Window.Location.getHost()+"/ITProjektSS17local.html", "_self", "");
						}
					});
					/*
					 * Eine neue Instanz der Klasse Navigation wird erstellt und
					 * dem RootPanel angefuegt
					 */
					RootPanel.get("Navi").clear();
