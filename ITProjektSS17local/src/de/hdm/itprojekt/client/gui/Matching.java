...
/**
 * Klasse die die Ausschreibungen anzeigt, die mit dem Profil des Benutzer
 * übereinstimmen
 * 
...
public class Matching extends VerticalPanel {
	...
	// Button erstellen
	private Button ausschreibungAnzeigenButton = new Button("Ausschreibung anzeigen & bewerben",
	...
	public Matching() {
		// matchingPanel dem mainPanel hinzufügen, sowie Labels,ListBox und
		// Button hinzufügen
		...
		ClientSideSettings.getProjektAdministration()
				.getProfilIdCurrentUser(ClientSideSettings.getCurrentUser().getId(), new FindProfilCallback());
	...
	private class FindProfilCallback implements AsyncCallback<Profil> {
		...
		public void onSuccess(Profil result) {
			...
			ClientSideSettings.getProjektAdministration().matchingAusschreibung(result.getId(),
					new GetMatchingAusschreibungenCallback());
	...
	private class AusschreibungAnzeigenClickHandler implements ClickHandler {
		...
		public void onClick(ClickEvent event) {
			...
			try {
				// Ausschreibungs Objekt, dass mit dem in der ListBox gewählten
				// Objekt gefüllt wird.
				...
				a.setBewerbungsfrist(
						aVector.elementAt(passendeAusschreibungListBox.getSelectedIndex()).getBewerbungsfrist());
				a.setProfil_idSuchprofil(
						aVector.elementAt(passendeAusschreibungListBox.getSelectedIndex()).getProfil_idSuchprofil());
